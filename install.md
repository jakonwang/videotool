...
**寻款拍照 / 导入描述（豆包，必选能力）：** 无需额外 Composer 包，使用 Guzzle 调方舟 **Chat Completions**（`/chat/completions`），与官方 SDK 示例里的 **`multimodal_embeddings`（向量）** 不是同一路径。环境变量示例：`VOLC_ACCESS_KEY`（Bearer）、**`VOLC_ARK_MODEL`**（默认推荐 **`doubao-1-5-vision-pro-32k`**，对应 **Doubao-1.5-vision-pro-32k**，以控制台「模型列表」完整 ID 为准）、`VOLC_ENDPOINT_ID`（**ep-** 接入点，有则优先于配置默认 model）、`VOLC_ARK_BASE_URL`、`VOLC_ARK_VERIFY_SSL`；全自动寻款还可选 **`VOLC_ARK_AUTO_CATALOG`**（注入豆包的候选条数，默认 **50**）、**`VOLC_ARK_AUTO_MATCH_MAX_TOKENS`**（单行编号输出长度上限，默认 **64**）。后台 **设置 → 豆包视觉** 填写并启用。排查时可执行 **`php scripts/volc_ark_ping.php`**（Windows：`php scripts\volc_ark_ping.php`）做文本对话连通性测试。调用后可在 **`runtime/log`** 检索 **`[volc_ark] 豆包`** 确认是否请求成功；失败时日志含 **`ark_error`** 或 **`curl_errno`/`curl_error`**；详见 `requirements.md`。
...
server {
    ...
    # 后台寻款等大导入：逐行调 Python，易超默认 60s 导致 502，按需调大
