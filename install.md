...
**寻款拍照 / 导入描述（豆包，必选能力）：** 无需额外 Composer 包，使用 Guzzle 调方舟 **Chat Completions**（`/chat/completions`），与官方 SDK 示例里的 **`multimodal_embeddings`（向量）** 不是同一路径。环境变量示例：`VOLC_ACCESS_KEY`（方舟 **API Key**，Bearer）、`VOLC_ENDPOINT_ID`（**对话类多模态**推理接入点 **ep-** ID）、`VOLC_ARK_BASE_URL`（默认 `https://ark.cn-beijing.volces.com/api/v3`）、**`VOLC_ARK_VERIFY_SSL`**（HTTPS 校验，默认开启；本机 SSL 报错时可临时 `false` 排查）。后台 **设置 → 豆包视觉** 填写并启用。调用后可在 **`runtime/log`** 检索 **`[volc_ark] 豆包`** 确认是否请求成功；失败时日志含 **`ark_error`** 或 **`curl_errno`/`curl_error`**；详见 `requirements.md`。
