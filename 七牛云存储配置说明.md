...
**推荐**：在管理后台侧栏 **系统 → 设置** 中维护 Access Key、Secret Key、Bucket、访问域名、区域及 **额外 CDN 域名（cdn_domains）**；与 `config/qiniu.php` / 环境变量 **合并**，数据库里某一项非空时覆盖对应文件配置。密钥类输入框**留空表示不修改**已保存的后台值；需改回仅用配置文件时勾选「清空」。**额外 CDN** 为多行或逗号分隔的域名/URL；仅当该文本框**非空**时替换配置文件中的 `cdn_domains` 列表，留空则仍使用配置文件。
