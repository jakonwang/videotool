...
/**
 * OpenAI gpt-4o-mini Vision：库内饰品（耳环、手链、项链等）特征描述 + 实拍图语义匹配
 */
class VisionSearchService
{
    ...
    public static function describeEarringImage(string $absolutePath): ?string
    {
        ...
        $body = [
            ...
            'messages' => [
                [
                    'role' => 'system',
                    'content' => '你是珠宝电商视觉标注员。只输出一行中文，50～140 字。请先判断品类（耳环/耳钉/耳坠、手链/手镯、项链/吊坠、戒指等饰品），再写清：金属色与光泽、主石/镶嵌（锆石/珍珠/贝母/无等）、造型结构（如耳针款、链节形态、吊坠轮廓）、风格（极简/复古/轻奢等）。手链项链可提链型与扣头；耳环可提耳针/耳圈形态。不要编号、不要价格、不要营销语。',
                ],
                [
                    'role' => 'user',
                    'content' => [
    ...
    public static function matchPhotoToCatalog(string $absolutePath, array $catalog): array
    {
        ...
        $instruction = <<<TXT
下面是饰品款式库的「产品编号|视觉特征描述|可选爆款备注」列表（每行一条；品类可含耳环、手链、项链等）。另附一张用户**实拍**照片。

任务：
1. 判断实拍主体是否为**饰品**（耳环/耳饰、手链、手镯、项链、吊坠、戒指等）；若完全无法辨认饰品主体，matches 为空数组。
2. 结合品类、颜色、金属质感、造型结构、镶嵌与风格，从列表中选出**最多 5 个**最可能对应的**产品编号**（只能从列表已出现的编号中选，禁止编造编号）。
3. 为每项给出 0～1 的置信度 score（浮点数），并简短说明 reason（中文，24 字内，可点明品类与相似点）。
...
TXT;
        ...
        $body = [
            ...
            'messages' => [
                [
                    'role' => 'system',
                    'content' => '你是饰品识款助手（耳环、手链、项链等）。只输出合法 JSON，键名使用英文 product_code、score、reason、matches、note。',
                ],
                [
                    'role' => 'user',
                    'content' => [
