...
/**
 * 以图搜款：Google → 火山方舟豆包视觉 → OpenAI Vision → 阿里云图像搜索
 */
class Search extends BaseController
{
    ...
    public function searchByImage()
    {
        ...
        return $this->jsonOut([
            'code' => 1,
            'msg' => '未配置寻款引擎：请在后台「设置」启用 Google / 火山方舟豆包 / OpenAI / 阿里云之一',
            'data' => null,
        ]);
    }
    ...
    private function searchByVolcArkVision(string $tmp, string $hint)
    {
        ...
        if ($catalog === []) {
            return $this->jsonOut([
                'code' => 1,
                'msg' => '库内无可用描述：请先在导入表格中填写「爆款类型」或通过 OpenAI 导入生成「视觉特征」，再使用豆包寻款。',
    ...
    private function searchByOpenAiVision(string $tmp, array $visionCfg)
    {
        ...
        if ($items === []) {
            return $this->jsonOut([
                'code' => 1,
                'msg' => '未从实拍图中匹配到库内编号，请靠近拍摄、保证饰品主体清晰，或检查库内特征是否与实物一致。',
