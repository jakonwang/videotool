...
**TikStar OPS**（本仓库）是基于 ThinkPHP 6 的**运营中台**：在统一后台中提供 **寻款**（商品/款式图搜索索引）、**达人运营**（TikTok 名录与达人链分发）、**素材库**（视频与商品）、**终端**（平台/设备）与系统管理。支持 TikTok、虾皮等多平台视频批量上传与按设备分配未下载素材。

模块划分与侧栏说明见 **`docs/requirements.md`**（与根目录 **`requirements.md`** 内容一致；维护时请以 `docs/requirements.md` 为 UTF-8 正本，见该文档末尾说明）开篇「TikStar OPS 系统模块」。
...
- ✅ **TikStar OPS 达人名录**：TikTok `@handle` 导入/编辑、达人链可选关联；详见根目录 `requirements.md`「品牌与达人 CRM」
...
- **后台**：`public/static/i18n/i18n.js`（全局 `AppI18n`，侧栏等用 `data-i18n`）。
...
- 当前全站 `i18n.js` / `influencer_i18n.js` 的 `?v=` 示例版本号：**`20260406_category1`**（修改字典后请再次统一 bump）。更细的维护说明见 **`docs/requirements.md`** / 根目录 **`requirements.md`**（「多语言」及「多语言切换」相关小节）。
...
- 后台入口：`/admin.php/profit_center`
- 目标：支持 TikTok 多店铺、多账户、多币种（USD/VND/CNY）日利润录入与汇总，统一换算到 `CNY`。
- 主要能力：
  - 店铺默认参数管理（售价/成本/取消率/平台扣点/达人佣金/时薪）
  - 店铺取消率支持按渠道独立配置（直播/视频/达人）
  - 账户管理（渠道、广告币种、默认 GMV 币种）
  - 日录入与利润计算（直播/视频/达人）
  - 汇率自动拉取与失败回退（`fallback_latest` 告警）
  - 导入模板下载（直播/视频/达人三 sheet）
  - Excel 导入（兼容 `直播GMV利润表/视频GMV利润表/达人GMV利润表`）
  - CSV 导出

### 迁移命令

- Windows：
  - `php database\\run_migration_profit_center.php`
- Linux：
  - `php database/run_migration_profit_center.php`
...
## 浏览器插件自动回传（Profit Center V1，2026-04-19）

- 插件目录：`tools/browser_plugin/profit_center_capture`
- 目标：在 TikTok Ads / TikTok Shop 页面抓取广告费、GMV、订单数并批量回传利润中心。

### 加载插件
1. 打开 `chrome://extensions` 或 `edge://extensions`
2. 开启“开发者模式”
3. 选择“加载已解压的扩展程序”，目录指向：
...
### 迁移与测试命令
- 迁移：`php database/run_migration_profit_plugin.php`
- 解析测试：`node scripts/profit_plugin_parser_test.js`
- 利润中心冒烟：`powershell -ExecutionPolicy Bypass -File scripts/profit_center_smoke.ps1`
