...
- **与官方 Python「多模态向量」示例的区别**：火山文档中 **`volcenginesdkarkruntime` → `client.multimodal_embeddings.create`**（如模型 `doubao-embedding-vision-*`）走的是 **`/embeddings` 类多模态向量接口**，与本项目 **无关**。寻款需要的是 **对话/视觉理解**：请在方舟控制台创建 **支持 Chat Completions 的多模态推理接入点**（`ep-` 开头），使 `POST {base_url}/chat/completions` 可用；若误用「仅 Embedding」接入点或把 **模型名** 当成 **`model`** 填错，会返回 **HTTP 4xx**，日志里 **`[volc_ark] 豆包 HTTP 失败`** 的 **`ark_error`** / **`body_snip`** 会写明原因。导入时 **`describeForImport`** 会先调 **`describe_import_fingerprint`** 再必要时调 **`describe_earring`**，因此同一行可能出现 **两次**「豆包请求开始」，属正常。
